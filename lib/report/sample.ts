 ...
 * 맛보기(공개) 샘플 리포트 빌더 — 스크립트(scripts/sample-report.ts)와
 * 웹 라우트(app/sample)가 같은 결과물을 쓰도록 로직을 한곳에 모은다.
 *
 * 프로덕션 파이프라인과 동일한 경로를 탄다:
 *   computeSaju(실계산) → generateReport(guardrails + 데이터 표 + 템플릿 조립) → renderReportHtml
 * 단, LLM 관점 산문만 미리 작성된 텍스트로 주입한다(LLM 키 불필요).
 */

import { computeSaju } from "../saju";
import { generateReport, renderReportHtml } from ".";
...
// 사주 실계산 — 2020-09-16 16:43 남 (壬戌일주 · 금38 수25 토25 목13 화0)
const SAMPLE_SAJU = computeSaju({
  birthYear: 2020,
  birthMonth: 9,
  birthDay: 16,
  birthHour: 16,
...
// 관점 산문 — 실서비스에서 Claude API가 작성하는 부분
  ...
  async complete() {
    return JSON.stringify({
      ...
      studyStyleProse: prose([
        ...
        "것일 가능성이 큽니다. 난이도를 잘게 쪼개 '쉽게 성공하는 첫 계단'을 만들어 주면 다음",
      ...
      majorProse: prose([
        ...
        "**이학·공학 계열** (수학·물리, 컴퓨터·데이터과학) — 깊이 사유하고 구조를 다루는 수(水)·금(金)의",
        "결과 잘 맞습니다.\n\n",
        "**탐구·전문 계열** (의·약학, 법학, 경제) — 정밀하게 파고들어 원칙을 세우는 기질과 연결됩니다.\n\n",
        "**인문·연구 계열** (철학, 국제학, 언어) — 한 주제를 끝까지 사유하는 강점을 살릴 수 있습니다.\n\n",
        "진학 환경으로는, 차분히 깊게 몰입할 수 있는 곳에서 힘을 내는 기질이라 급격한 변화보다 충분한",
        "탐구가 가능한 환경이 어울리는 경향입니다. 국내 진학이든 해외 유학이든, 관심 전공이 또렷해진 뒤",
        "**그 분야가 강한 국내외 대학을 직접 살펴보시기**를 권합니다. 특정 대학을 사주로 정하는 것이",
        "아니며, 진학·유학 방향은 아이의 흥미와 가정 여건을 함께 고려해 결정하실 참고자료입니다.",
      ]),
      ...
      eduStagesProse: prose([
        "**유아기** — 지금 이 시기의 핵심은 '공부'가 아니라 마음껏 몰입하는 경험입니다. 좋아하는 놀이에",
        "충분히 빠져 보게 하고, 노래·율동·역할놀이처럼 밖으로 표현하는 활동을 곁들여 옅은 화(火)의",
        "표현력을 자연스럽게 북돋아 주세요. 이 시기의 몰입 경험이 평생 학습 태도의 뿌리가 됩니다.\n\n",
        "**초등** — 성적보다 '학교는 안전하고 예측 가능한 곳'이라는 감각과, 좋아하는 분야를 깊게 파 보는",
        "경험이 중요합니다. 강한 탐구 기질을 살려 한 분야(공룡·우주·지하철 무엇이든)를 마음껏 파고들게",
        "해 주면, 그 몰입이 자기주도 학습의 밑돌이 됩니다.\n\n",
        "**중등** — 첫 시험과 내신이 시작됩니다. 점수 자체보다 '계획을 세우고-실행하고-틀린 것을 복기하는'",
        "틀을 몸에 익히도록 돕는 것이 이 아이의 분석 강점과 잘 맞습니다. 자유학기 활동은 조용히 관찰·",
        "기록하는 역할부터 맡겨 자신감을 쌓게 해 주세요.\n\n",
        "**고등** — 학습 전략의 비중이 커지는 시기입니다. 이 아이는 자기에게 맞는 공부법(깊이 이해 후",
        "구조화)을 갖추게 될 결이므로, 그 방식을 신뢰하고 충분한 시간을 확보해 주는 것이 중요합니다.",
        "체력·수면 관리가 곧 성적 관리인 시기이기도 합니다.\n\n",
        "**대학·진로** — 오래 흥미를 둔 분야가 전공 선택의 단서가 됩니다. 깊이 파고드는 수(水)·금(金)의",
        "결은 연구·전문 분야와 잘 맞는 경향이 있으니, 관심 전공을 좁혀 가며 그 전공이 어떤 직업으로",
        "이어질 수 있는지 스스로 그려 보도록 곁에서 도와주시면 좋습니다. 특정 진로를 정해 주기보다,",
        "탐색의 방향을 함께 넓혀 가는 것이 이 아이 기질에 맞습니다.",
      ]),
      daeunProse: prose([
